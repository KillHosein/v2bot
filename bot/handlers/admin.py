...
async def admin_orders_pending(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Show pending orders for review"""
    query = update.callback_query
    await query.answer()
    ...
    pending_orders = query_db(
        """SELECT o.id, o.user_id, o.timestamp, p.name as plan_name, 
           COALESCE(o.final_price, p.price) as price, u.first_name
           FROM orders o
           LEFT JOIN plans p ON p.id = o.plan_id
    ...
    if not pending_orders:
        text = "✅ هیچ سفارش در انتظار تأییدی وجود ندارد."
        keyboard = [[InlineKeyboardButton("🔙 بازگشت", callback_data='admin_orders_menu')]]
    else:
        text = f"⏳ <b>سفارشات در انتظار تأیید</b>\n\n📊 تعداد: {len(pending_orders)}\n\n"
        keyboard = []
        
        for order in pending_orders:
            user_name = order.get('first_name', 'کاربر')
            plan = order.get('plan_name', 'نامشخص')
            price = order.get('price', 0)
            timestamp = order.get('timestamp', '')
            
            text += f"🔸 سفارش #{order['id']}\n"
            text += f"   👤 {user_name} (ID: {order['user_id']})\n"
            text += f"   📦 {plan}\n"
            text += f"   💰 {int(price):,} تومان\n"
            text += f"   🕐 {timestamp}\n\n"
