...
async def my_services_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    expired_emoji = "🔴" if expired_count > 0 else "⚪"
    
    text = (
        f"📱 <b>سرویس‌های من</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📊 <b>خلاصه وضعیت:</b>\n\n"
        f"{active_emoji} <b>فعال:</b> {active_count} عدد\n"
        f"{pending_emoji} <b>در انتظار:</b> {pending_count} عدد\n"
        f"{expired_emoji} <b>منقضی شده:</b> {expired_count} عدد\n"
        f"📦 <b>کل سرویس‌ها:</b> {len(orders)} عدد\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    )
    
    if total_pages > 1:
        text += f"📄 <i>صفحه {page} از {total_pages}</i>\n\n"
...
async def usage_stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not services:
        ...
        for service in services[:10]:  # Show max 10 services
...
async def user_settings_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        "⚙️ <b>تنظیمات پیشرفته کاربر</b>\n\n"
        "🚀 <b>ویژگی‌های جدید v3.0:</b>\n"
        "   🧠 هوش مصنوعی و تجزیه رفتار\n"
        "   📱 اپلیکیشن موبایل پیشرفته\n"
        "   ⛓️ ادغام بلاک‌چین و DeFi\n"
    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton("🔔 اعلان‌ها", callback_data='notifications_settings')
        ],
        [
            InlineKeyboardButton("📊 آمار استفاده", callback_data='usage_stats'),
            InlineKeyboardButton("🧠 تجزیه رفتار AI", callback_data='ai_behavior_analysis')
        ],
        [
            InlineKeyboardButton("📱 اپ موبایل", callback_data='qr_code_generator'),
            InlineKeyboardButton("⛓️ بلاک‌چین", callback_data='blockchain_integration')
        ],
        [InlineKeyboardButton("🔙 بازگشت", callback_data='start_main')]
    ]
...
async def notifications_settings_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get current notification settings (assume default values if not set)
    settings = query_db(
