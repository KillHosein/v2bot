...
def db_setup():
    with sqlite3.connect(DB_NAME, check_same_thread=False) as conn:
        ...
        try:
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_orders_panel_status ON orders(panel_id, status)")
        except sqlite3.Error:
            pass
        try:
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_orders_username ON orders(marzban_username)")
        except sqlite3.Error:
            pass
        try:
