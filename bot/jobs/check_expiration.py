...
async def backup_and_send_to_admins(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as z:
            ...
            z.writestr('settings.json', data=json.dumps(settings, ensure_ascii=False, indent=2))
            
            # Users
            users = query_db("SELECT * FROM users") or []
            z.writestr('users.json', data=json.dumps(users, ensure_ascii=False, indent=2, default=str))
            
            # Orders
            orders = query_db("SELECT * FROM orders") or []
            z.writestr('orders.json', data=json.dumps(orders, ensure_ascii=False, indent=2, default=str))
            
            # Plans
            plans = query_db("SELECT * FROM plans") or []
            z.writestr('plans.json', data=json.dumps(plans, ensure_ascii=False, indent=2, default=str))
            
            # Panels
            panels = query_db("SELECT * FROM panels") or []
            z.writestr('panels.json', data=json.dumps(panels, ensure_ascii=False, indent=2, default=str))
            
            # Wallet Transactions
            wallet_txs = query_db("SELECT * FROM wallet_transactions") or []
            z.writestr('wallet_transactions.json', data=json.dumps(wallet_txs, ensure_ascii=False, indent=2, default=str))
            
            # Tickets
            tickets = query_db("SELECT * FROM tickets") or []
            z.writestr('tickets.json', data=json.dumps(tickets, ensure_ascii=False, indent=2, default=str))
            
            # Discount Codes
            discounts = query_db("SELECT * FROM discount_codes") or []
            z.writestr('discount_codes.json', data=json.dumps(discounts, ensure_ascii=False, indent=2, default=str))
            
            # Cards
            cards = query_db("SELECT * FROM cards") or []
            z.writestr('cards.json', data=json.dumps(cards, ensure_ascii=False, indent=2, default=str))
            
            # Referrals
            referrals = query_db("SELECT * FROM referrals") or []
            z.writestr('referrals.json', data=json.dumps(referrals, ensure_ascii=False, indent=2, default=str))
