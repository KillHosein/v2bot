    ...
    try:
        ...
        for order in orders:
            try:
                ...
                # Get service stats from panel
                api = VpnPanelAPI(panel)
...
async def send_traffic_warning(bot, user_id, order_id, plan_name, usage_percent, used_gb, total_gb, level='warning'):
    ...
    text = (
        f"{icon} <b>{title}</b>\n\n"
        f"📦 <b>پلن:</b> {plan_name}\n"
        f"📊 <b>حجم مصرفی:</b> {used_gb:.2f} GB از {total_gb:.0f} GB\n"
        f"📈 <b>درصد مصرف:</b> {usage_percent:.1f}%\n\n"
        f"💡 برای جلوگیری از قطعی سرویس، همین حالا تمدید کنید!"
    )
...
async def send_expiry_warning(bot, user_id, order_id, plan_name, days_left, expiry_date, level='warning', hours=None):
    ...
    text = (
        f"{icon} <b>{title}</b>\n\n"
        f"📦 <b>پلن:</b> {plan_name}\n"
        f"⏳ <b>زمان باقیمانده:</b> {time_text}\n"
        f"📅 <b>تاریخ انقضا:</b> {expiry_str}\n\n"
        f"💡 برای ادامه استفاده، همین حالا سرویس خود را تمدید کنید!"
    )
